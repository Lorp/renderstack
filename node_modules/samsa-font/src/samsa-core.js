...
class SamsaBuffer extends DataView {
	...
	decodePaint(context = {}) {
		...
		// - set is at least as fast as sparse array
		if (context.paintIds.has(paint.offset)) {
			console.error("decodePaint(): infinite recursion detected, paintId", paint.offset, "already used in this paint");
			return paint; // return what we have found so far
		}
		else {
			context.paintIds.add(paint.offset);
		}
...
SamsaGlyph.prototype.paint = function (context={}) {
	...
	if (!offset) {
		...
		context.paintIds = new Set(); // keep track of the paint IDs we’ve used (must be reset for each glyph)
