"use strict";
// samsa-core-2.js
// ~/node_modules/uglify-js/bin/uglifyjs samsa-core-2.js > samsa-core-2.min.js
...
const TABLE_DECODERS = {
	...
	"avar": (font, buf) => {
		...
		if (font.avar.version[0] == 2) {
			...
			console.log("this.avar.axisIndexMap",font.avar.axisIndexMap);
		}
	},
	...
	"cmap": (font, buf) => {
		...
		if (cmap.encodingRecord) {
			...
			switch (characterMap.format) {
				...
				default:
					console.log(`Warning: Not handling cmap format ${format}`);
					break;
			}
	...
	"COLR": (font, buf) => {
		...
		if (colr.version <= 1) {
			...
			// should we be caching any of this stuff?
			//colr.paints = []; // the main array for storing structured COLRv1 data... the cool thing is that it is sparse, with offsets as the indices TODO: try an object to see if it’s faster
			//colr.cache = []; // cache of paintRecords, indexed by their offset (so it’s sparse)
	...
	"fvar": (font, buf) => {
		...
		for (let i=0; i<fvar.instanceCount; i++) {
			fvar.instances.push(buf.decode(FORMATS.InstanceRecord, fvar.axisCount, includePostScriptNameID));
		}
	},
...
//			console.log(`Getting ashared tuple ${t}`);
	...
	"hmtx": (font, buf) => {
		...
		if (g < font.maxp.numGlyphs) {
			for ( ; g<font.maxp.numGlyphs; g++) {
	...
	"vmtx": (font, buf) => {
		...
		font.vmtx = [];
		let g;
		for (g=0; g<numOfLongVerMetrics; g++) {
			font.vmtx[g] = buf.u16;
			buf.seekr(2); // skip over tsb, we only record advance height
		}
		if (g < font.maxp.numGlyphs) {
			for ( ; g<font.maxp.numGlyphs; g++) {
				font.vmtx[g] = font.vmtx[numOfLongVerMetrics-1];
			}
		}
	},
}
...
function validateTuple (tuple, axisCount) {
	...
	for (let a=0; a < axisCount; a++)
 		//if (typeof tuple[a] != "number" || tuple[a] < -1 || tuple[a] > 1)
...
function processPaintSVG (paint, rendering) {
	...
	// keep track of a max paint depth?

}


// console.log(gradient.points); console.log([finalX, finalY]);
// console.log("rotLength, dotProd,magnitude, finalX, finalY", rotLength, dotProd, magnitude, finalX, finalY);


		// fix the gradients
		/*
		Object.keys(extra.gradients).forEach((gradientId) => {
			const gradient = extra.gradients[gradientId]; // gradientIds are strings e.g. "g1235"


			// extra.gradients.forEach((gradient, gradientId) => {
			...
			// calculate the gradient line from the 3 points by determining the projection vector
			const
				p0x = gradient.points[0][0],
				p0y = gradient.points[0][1],
				p1x = gradient.points[1][0],
				p1y = gradient.points[1][1],
				p2x = gradient.points[2][0],
				p2y = gradient.points[2][1],
				d1x = p1x - p0x,
				d1y = p1y - p0y,
				d2x = p2x - p0x,
				d2y = p2y - p0y,
			...
			defs += `<linearGradient id="${gradientId}" gradientUnits="userSpaceOnUse" x1="${p0x}" y1="${p0y}" x2="${finalX}" y2="${finalY}">`;
			gradient.colorLine.colorStops.forEach(stop => {
				defs += `<stop offset="${stop.stopOffset * 100}%" stop-color="#${bgraToHexColor(palette.colors[stop.paletteIndex])}" />`;
			});
			defs += `</linearGradient>`;
...
// assign the null functions to the last existing previous 
...
// - there are decode/encode methods for converting complex data structures to and from binary
class SamsaBuffer extends DataView {
	...
	decodeGlyph(options={}) {
		...
		if (options.length) {
			...
			// const init = {};
			// init.numberOfContours = 0,
			// init.numPoints = 0;
			//init.points = [[0,0,0],[metrics[1],0,0],[0,metrics[2],0],[0,0,0]];
			//return new SamsaGlyph(init);
			glyph = new SamsaGlyph(); // makes an empty glyph
			//return new SamsaGlyph({ numberOfContours: 0, points: [] });
		}
		*/
		
		glyph.id = options.id;
		//glyph.points = []; // already handled by SamsaGlyph constructor
	...
	// - deltaSetIndexMaps are always 0-based, so do not omit any items (HMM, really?)
	decodeIndexMap() {
		const indexMap = [];
	...
	decodeTvts(glyph) {
		...
		//const buf = font.bufferFromTable("gvar"); // hmm, what is "this" here? teh font’s buffer or the gvar buffer? we should really decode the tvt directly from the gvar buffer
	...
	/*
	// TODO: avoid this here, make it inside decodePaint()
	decodeColorLine(paint) {
		const colorLine = {
			extend: this.u8,
			numStops: this.u16,
			colorStops: [],
		...
		for (let cst=0; cst<colorLine.numStops; cst++) {
			...
			// const colorStop = {
			// 	stopOffset: this.f214,
			// 	paletteIndex: this.u16,
			// 	alpha: this.f214,
			// };
			if (paint.format % 2) { // this test is valid for all the paint formats that call getColorLine() 
				colorStop.varIndexBase = this.u32;
			}
	...
	*/
	...
	// TODO: avoid this here, make it inside decodePaint()
	decodeTransform() {
		return [ this.f1616, this.f1616, this.f1616, this.f1616, this.f1616, this.f1616 ];
	}
	*/
	...
	decodePaint(context = {}) {
		...
		// - it’s an arrow function to keep "this" (the buffer) in scope
		const addVariations = (operands) => {
			...
			for (let i=0; i<operands.length; i++) {
				let outer, inner;
				let index = varIndexBase + i;
				if (index) { // hmm, is this correct? we might have varIndexBase == 0 and i == 0
					if (colr.varIndexMapOffset) {
						[outer, inner] = colr.varIndexMap[index];
					}
					else {
						outer = index >>> 16; // >>> avoids creating negative values (we want 0xffff, not -1)
						inner = index & 0xffff;
					}
		...
		/*
		// we don’t seem to use decodeTransform() at all, whether in the global scope or here in decodePaint()
		...
		*/
		...
		switch (paint.format) {
			...
			case 2: case 3: { // PaintSolid
				...
				operands.push(this.i16);
				addVariations(operands);
				paint.alpha = operands[0] / 16384;
				break;
			}

			case 4: case 5: { // PaintLinearGradient, PaintVarLinearGradient
				const colorLineOffset = this.u24;
				paint.colorLine = decodeColorLine(colorLineOffset);
				operands.push( this.i16, this.i16, this.i16, this.i16, this.i16, this.i16 );
				addVariations(operands);
			...
			case 8: case 9: { // PaintSweepGradient, PaintVarSweepGradient
				const colorLineOffset = this.u24;
				paint.colorLine = decodeColorLine(colorLineOffset);
				operands.push( this.i16, this.i16, this.i16, this.i16 ); // centerX, centerY, startAngle, endAngle
				addVariations(operands);
			...
			case 10: { // PaintGlyph
				// we got here, so we can use the transform that has been building up
			...
			case 12: case 13: { // PaintTransform, PaintVarTransform
				...
				operands.push( this.i32, this.i32, this.i32, this.i32, this.i32, this.i32 );
				this.seek(tell);
				addVariations(operands);
			...
			case 14: case 15: { // PaintTranslate, PaintVarTranslate
				const nextOffset = this.u24;
				operands.push( this.i16, this.i16 );
				addVariations(operands);
			...
			case 16: case 17: case 18: case 19: case 20: case 21: case 22: case 23: { // PaintScale and variant scaling formats
				const nextOffset = this.u24;
				if (16 <= paint.format && paint.format <= 19) {
					operands.push( this.i16, this.i16 ); // scaleX, scaleY
				}
				else {
					operands.push( this.i16 ); // scaleX, scaleY
				}
			...
			case 24: case 25: case 26: case 27: { // PaintRotate, PaintVarRotate, PaintRotateAroundCenter, PaintVarRotateAroundCenter
				const nextOffset = this.u24;
				operands.push( this.i16 );
				if (paint.format == 26 || paint.format == 27) {
					operands.push( this.i16, this.i16 );
				}
				addVariations(operands);
			...
			case 28: case 29: case 30: case 31: { // PaintSkew, PaintVarSkew, PaintSkewAroundCenter, PaintVarSkewAroundCenter
				const nextOffset = this.u24;
				operands.push( this.i16, this.i16 );
				if (paint.format == 30 || paint.format == 31) {
					operands.push( this.i16, this.i16 );
				}
				addVariations(operands);
			...
			default: {
				console.error(`ERROR: Unknown paint.format ${paint.format} (must be 1 <= paint.format <= 32)`)
				break;
			}
		}
...
function SamsaFont(buf, options = {}) {
	...
	this.tableList = [];
	this.glyphs = [];
	this.ItemVariationStores = {}; // keys will be "MVAR", "COLRv1", "CFF2", "HVAR", "VVAR"... they all get recalculated when a new instance is requested
	this.tableDecoders = TABLE_DECODERS;
	...
	ivsTags.forEach(tag => {
		if (this[tag] && this[tag].itemVariationStoreOffset) {
			console.log("ItemVariationStore:", tag);
	...
	if (options.allGlyphs) {
		//console.log("options.allGlyphs == true");
		let start, end;
		let offset = 0;
		start = performance.now();
		// we need to do this differently if the font is only lightly loaded, e.g. from a big file that we don’t want to load in full
		for (let g=0; g<this.maxp.numGlyphs; g++) {
			...
			//console.log("we got font ", this.glyphs[g].font)
			//console.log("we got font ", this)
			//this.glyphs[g].font = this;
			offset = nextOffset;
		}
		end = performance.now();
	...
	// we should NOT! callback in the SamsaFont constructor!
	// // callback, if any
	// if (options.callback) {
...
SamsaFont.prototype.u32ToHexColor = function (num) {
...
/*
string to SVG means
string to glyphRun, then glyphRun to glyphLayout, then glyphLayout to one piece of SVG
...
//////////////////////////////////
SamsaFont.prototype.fvsToTuple = function (fvs) {
...
function SamsaInstance(font, userTuple, options={}) {
	...
	//console.log("Creating an instance at ", tuple);
	...
	if (avar) {
		...
		if (avar.axisSegmentMaps) {
			for (let a=0; a<axisCount; a++) { // should use the local axisCount
				let n = tuple[a];
				const map = avar.axisSegmentMaps[a];
		...
		if (avar.itemVariationStore) {
			...
			// avar2 mappings
			//console.log("ItemVariationStore", "avar", "... making a final tuple for use in subsequent operations");
			...
			for (let a=0; a<axisCount; a++) {
				...
				if (outer != 0xffff && inner != 0xffff) { // if this entry is non-null... (hmm, might be nicer to have created a sparse array in the first place, skipping nulls and thus avoiding this check)
					console.log("Updating axis" + a + ":" + tuple[a] , "+=", Math.round(deltas[outer][inner]) / 0x4000);
					tuple[a] += Math.round(deltas[outer][inner]) / 0x4000; // add the interpolated delta to tup[axisId]
					tuple[a] = clamp(tuple[a], -1, 1); // clamp the result to [-1,1]
				}
			}
			//console.log("avar: transformed tuple", tuple);
		}
		else {
			//console.log ("No avar2 in this font");
		}
	}
	...
	Object.keys(font.ItemVariationStores).forEach(key => {
		...
		if (key != "avar") { // we already handled the avar2 ItemVariationStore, if it exists
			//console.log("ItemVariationStore", key, "... instantiating");
			
			const deltaSets = font.itemVariationStoreInstantiate(font.ItemVariationStores[key], tuple);
			// this.deltaSets[key] = font.itemVariationStoreInstantiate(font.ItemVariationStores[key], tuple);

			// we need to keep this sticking around? we don’t need to for MVAR, as we can get the new values

			// each ivs has its own way of mapping indices
			// the mapping can be done JIT... maybe, but maybe not
...
SamsaInstance.prototype.glyphAdvance = function (glyphId) {
	...
	//console.log("Doing glyphId" , glyphId)
...
SamsaInstance.prototype.glyphLayoutFromString = function (input) {
	...
	glyphRun.forEach(glyphId => {
		...
		// get the font’s glyph
		if (!font.glyphs[glyphId]) {
			font.glyphs[glyphId] = new SamsaGlyph(this, glyphId);
		}

		// get the instance’s glyph
		if (!this.glyphs[glyphId]) {
			this.glyphs[glyphId] = new SamsaGlyph(this, glyphId);
		}

		const glyph = this.glyphs[glyphId];

		const [dx, dy] = this.glyphAdvance(glyphId);
...
SamsaGlyph.prototype.instantiate = function(arg, options={}) {
	...
	//else if (validateTuple (arg, this.font.axisCount)) {
	else if (validateTuple (arg, axisCount)) {
		//console.log("Yes2 it’s a tuple!!! DANGER: this glyph should really be part of an instance")
		iglyph.tuple = tuple;
		...
		//console.log("OH DEAR: no tuple, no instance, so we give it the null tuple")
		//iglyph.tuple = Array(this.font.axisCount).fill(0);
		...
		this.components.forEach((component, c) => {
			...
			if (this.instance) {
				...
				if (!gc) { // if it’s not already instantiated, instantiate it
					...
					//gc = gcDefault.instantiate(instance); // it still needs to be decomposed
...
//   .paths, array to cache the glyph outlines used in the text run, indexed by glyphId
//   .gradients, object to cache the gradients used, indexed by gradientId (which is 'f' + the offset of the gradient in COLR)
	...
	switch (PAINT_TYPES[paint.format]) {
		...
		case PAINT_SHAPE: {
			if (!context.paths[paint.glyphID]) {
				...
				//let iglyph = context.instance.glyphs[paint.glyphID];

				//const path = context.font.glyphs[paint.glyphID].svgGlyphMonochrome(0); // retrieve it if we don't already have it
		...
		case PAINT_TRANSFORM: {
			...
			if (paint.translate) {
				...
				if (paint.center) {
					transform += `translate(${-paint.center[0]} ${-paint.center[1]}) `;
				}
				if (paint.scale) {
		...
		case PAINT_COMPOSE: {
			switch (paint.format) {
				case 2: case 3: { // PaintSolid
					...
					svg += `<use x="0" y="0" href="#g${context.lastGlyphId}" fill="${font.u32ToHexColor(color)}" />`; // maybe update these x and y later
					break;
				}

				case 4: case 5: { // PaintLinearGradient
					const gradientId = `f${paint.offset}`; // needs to be uniquer? maybe not
					if (!context.gradients[gradientId]) {
						...
						let gradient = `<linearGradient id="${gradientId}" gradientUnits="userSpaceOnUse" x1="${p0x}" y1="${p0y}" x2="${finalX}" y2="${finalY}">`;
						paint.colorLine.colorStops.forEach(colorStop => {
							const paletteIndex = colorStop.paletteIndex;
							const color = paletteIndex == 0xffff ? context.color : palette.colors[paletteIndex];
							gradient += `<stop offset="${colorStop.stopOffset*100}%" stop-color="${font.u32ToHexColor(color)}"/>`;
						});
						gradient += `</linearGradient>`;
						context.gradients[gradientId] = gradient;
					}
					svg += `<use href="#g${context.lastGlyphId}" fill="url(#${gradientId})" />`; // we have x and y attributes available here, but we use the enclosing <g> instead
				...
				case 32: { // PaintComposite
					...
					const src = paintSVG(paint.children[0], context); // source
					const dest = paintSVG(paint.children[1], context); // destination
...
// each of these 3 function supplies the contents for the containing <g> element in the SVG
// - we can bring many of them together in a single SVG when getting a glyph run
// - should we be keeping track of gradient ids (integers), not defs?
SamsaGlyph.prototype.svgGlyphCOLRv1_OLD = function (context={}) {
	...
	if (!context.font) context.font = font;
	if (!context.rendering) context.rendering = {};
	//const buf = new SamsaBuffer(font.buf.buffer, font.tables["COLR"].offset, font.tables["COLR"].length);
	const buf = font.bufferFromTable("COLR");
	const colr = font.COLR;
	const cpal = font.CPAL;
	const gradients = context.gradients;
	...
	const offset = this.findCOLR(1);
	const palette = cpal.palettes[context.paletteId || 0];
	let color = context.color || 0x000000ff; // {b: 0, g: 0, r: 0, a: 255}; // keep this in the same context as main transform, font size, user palette selection
	let svgElements = "";

	context.shape = [];
	context.rendering.depth = 0;
	context.rendering.transform = [];

	function renderSVG_handler(context, paint) {
		// custom way to handle paints, just for SVG
		// - we’d write a seperate one of these for canvas, for example
		context.rendering.depth ++;
		console.log("Depth: " + context.rendering.depth);
		console.log(paint);
		let svgString = "";
		...
		switch (paint.type) {
			...
			case PAINT_LAYERS:
				//console.log(`<g class="PAINT_LAYERS DEPTH(${rendering.depth})" transform="${rendering.transform.join(" ")}">\n`);
				console.log(`<g class="PAINT_LAYERS DEPTH(${context.rendering.depth})" transform="?">\n`);

				paint.children.forEach(child => {
					//const newoff = child // colr.layerListOffset + child;
					buf.seek(child);
					//let newpaint = buf.decodePaint(colr); // this needs a new rendering object, which will be parsed when we get to a terminator
					let newpaint = buf.decodePaint(context); // this needs a new rendering object, which will be parsed when we get to a terminator
					console.log("  child paint", newpaint);
					svgString += renderSVG_handler(context, newpaint);
					//console.log(newpaint);
				});
				//colr.layerListOffset + colr.layerList[paint.firstLayerIndex + lyr]
				console.log(`</g> <!-- PAINT_LAYERS -->\n`);
				context.rendering.depth--;
				break;

			case PAINT_TRANSFORM:
				// set up matrix, scale, rotate, translate, skew
				// - hmm... we could just have all parameters in an array, then the custom function arranges them if necessary
				// - this would handle the optional centerX, centerY etc.

				// - and we can prepopulate an array
				// - const transformType = [ "matrix", "scale", "rotate", "translate", "skew" ] with appropriate indices and duplicate entries for paint.format lookups
				// - rendering.transform.push(`${transformType}(${paint.transform.join(" ")})`);


				if (paint.matrix) {
					context.rendering.transform.push(`matrix(${paint.matrix.join(" ")})`);
				}
				else if (paint.translate) {
					context.rendering.transform.push(`translate(${paint.translate.join(" ")})`);
				}
				else if (paint.rotate) {
					context.rendering.transform.push(`rotate(${paint.rotate})`);
				}
				else if (paint.scale) {
					context.rendering.transform.push(`scale(${paint.scale.join(" ")})`);
				}
				else if (paint.skew) {
					context.rendering.transform.push(`skew(${paint.skew.join(" ")})`);
				}

				buf.seek(paint.children[0]);
			...
			case PAINT_SHAPE:
				// almost terminate (get final info)
				// 	renderSVG(paint, rendering);

				// according to the spec, we should now only have 


				/*
				paint.children.forEach(child => {
					//const newoff = child // colr.layerListOffset + child;
					//console.log("child", newoff);
					buf.seek(child);
					let newpaint = buf.decodePaint(colr); // this needs a new rendering object, which will be parsed when we get to a terminator
					//renderSVG_handler(newpaint, rendering);
					console.log(newpaint);
				});
				//colr.layerListOffset + colr.layerList[paint.firstLayerIndex + lyr]
...
//				console.log(`<g> <!-- PAINT_SHAPE -->\n`);

				// let newpaint1 = buf.decodePaint(context); // this needs a new rendering object, which will be parsed when we get to a terminator
				// renderSVG_handler(context, newpaint1);
				// console.log(newpaint1);
				//console.log(font.glyphs[paint.glyphID].points);



				const svgPath = `<path d="${font.glyphs[paint.glyphID].svgPath()}"/>`;
				
				//console.log(font.glyphs[paint.glyphID].svgPath());
				// console.log("Adding path");
				// console.log(svgPath);
				
				//console.log(`<g>${svgPath}</g> <!-- PAINT_SHAPE -->\n`);
				//svgString += `<g>${svgPath}</g> <!-- PAINT_SHAPE -->\n`;

				// we add this path to our sequence of shapes
				// - ultimately, one of them will be the main path, the others will be clip regions (it makes no difference which is chosen)

				// if (!context.shape) {
				// 	context.shape = [svgPath];
				// }
				// else {
				// 	context.shape.push(svgPath);
				// }
				context.shape.push(svgPath);

				// render the child, which is probably a fill or gradient, but may be another shape
				//let newpaint = buf.decodePaint(context); // this needs a new rendering object, which will be parsed when we get to a terminator

				buf.seek(paint.children[0]);
				svgString += renderSVG_handler(context, buf.decodePaint(context));


				// we still need to know how to fill this damn thing

				//console.log(`</g> <!-- PAINT_SHAPE -->\n`);

				// CLIPPING: if we have multiple shapes, we need to clip them. It shoudl be quite easy by making all but one of the paths a <clipPath id="X"> in the <defs>
				// - Note: this is also the simplest way to handle negative shapes

				break;

			case PAINT_END:
				// terminate
				// - fill or apply gradient or composite

				// we should have a shape by now! :D

				let fillConcept;

				if (context.shape) {
					console.log("YES we have a shape of length " + context.shape.length);
				}

				// fill with random color!
				console.log(`PAINT_TERMINATE! (depth=${context.rendering.depth})`);
				
				if (paint.format == 2 || paint.format == 3) {
					// PaintSolid, PaintVarSolid
					console.log("fill");
					fillConcept = `fill="${palette[paint.paletteIndex]}" fill-opacity="${paint.alpha}"`;
				}

				else if (paint.format == 32) {
					console.log("composite");

					// we need to recusrse twice into the tree (once for each child)
				}

				else {
					console.log("gradient");
					fillConcept = `fill="url(#f${paint.offset})"`; // we identify gradients by their paint.offset
					

					gradients[paint.offset] = paint; // store the gradient for later (we might do this same entry multiple times but that’s fine, it’s just a pointer to the same paint)

					// gradientId = "g" + grandchild.offset; // we rely on the font’s compiler to reuse identical gradients
					// console.log("FORMAT 4/5");
					// console.log(grandchild.points)
					// console.log(grandchild.colorLine)

					// // convert the points to SVG gradient stops
					// if (!extra.gradients[gradientId]) {
					// 	extra.gradients[gradientId] = {
					// 		colorLine: grandchild.colorLine,
					// 		points: grandchild.points,
					// 	}; // sparse array (NO, now it’s an object with keys like "g2345")

					// }

				}

				svgString += `<g ${fillConcept}>`;
				svgString += context.shape[0];
				svgString += "</g>";

				// insert the gradient into defs


				//let glyph = rendering.pop();
				
				
				
				//console.log(`<g transform="${glyph.transform.join(" ")}"> <path></path> <path></path> </g>`);

				//console.log("</g>");

				context.shape = []; //.length = 0; // empty the shape array, as we have rendered it
				break;
		}

		//let result = `<g transform="${rendering.transform.join(" ")}"> <path></path> <path></path> </g>`;
		console.log("within ", svgString)
		return svgString;
	}

	if (offset) {
		console.log(`G${this.id}: COLRv1`);
		const color = context.color || 0x000000ff; // default color

		buf.seek(offset+2);
		//let paintOffset = colr.baseGlyphListOffset + buf.u32;
		let paintOffset = colr.baseGlyphPaintRecords[this.id];


		console.log(`Getting COLRv1 glyph ${this.id} @${paintOffset}`)

		buf.seek(paintOffset);

		console.log("!PaintOffset = " + paintOffset)
		// process.exit();

		// now we are at a root Paint offset

		// parse the paint recursively
		// each item in the rendering stack has a transform and at some points needs to be ended
		const rendering = []; // stack, where the topmost item receives transforms, shapes and terminators
		//const rendering = []; // or stack, where the <g>’s keep nesting ever deeper


		// !!!!!! MAIN CALL !!!
		//let paint = buf.decodePaint(font);
		let paint = buf.decodePaint(context);
		
		svgElements = renderSVG_handler(context, paint);



		// TODO: this is not the right place to do this, it should be done when we wrap the SVG finally
		// now we can fix up the returned SVG string
		// - the gradients are guaranteed to be unique, so we can just insert them into the SVG
		gradients.forEach((gradient, gradientId) => {
			const [p0x, p0y] = gradient.points[0];
			const [finalX, finalY] = font.linearGradientFromThreePoints(gradient.points);
			let gradientSVG = `<linearGradient id="f${gradientId}" gradientUnits="userSpaceOnUse" x1="${p0x}" y1="${p0y}" x2="${finalX}" y2="${finalY}">`;
			gradient.colorLine.colorStops.forEach(stop => {
				gradientSVG += `<stop offset="${stop.stopOffset * 100}%" stop-color="${font.u32ToHexColor(palette.colors[stop.paletteIndex])}"/>`;
			});
			gradientSVG += `</linearGradient>`;
			context.defs.push(gradientSVG);
		});
		...
		// if (defs.length) {
		// 	svgElements = svgElements.replace("<defs></defs>", `<defs>${defs.join()}</defs>`);
		// }
	
		
		console.log(context.defs)
//		process.exit();


		// if it’s a root, we start a <g>
		// if it’s a fill, we return a <g>
		// if it’s a transform, we add the transform to the current queue and go to the next paint [or do we?]
		// if it’s a glyph, we can add our outline points
		// if it’s a layerlist, we step thru each of the items in the layerlist as a separate <g>

		//return `<g class="COLRv1@${offset}">\n${elements}</g>\n`;
...
SamsaGlyph.prototype.svgGlyphCOLRv0 = function (context={}) { // we need to poss the instance, right?
	...
	if (result) {
		...
		for (let i=0; i<numLayers; i++) {
			...
			const layerGlyph = font.glyphs[glyphId]; // instantiate this
			if (layerGlyph.numberOfContours > 0) {
				...
				elements += `<path d="${layerGlyph.svgPath()}" fill="${font.u32ToHexColor(layerColor)}"/>\n`; // TODO: precalculate the hex strings for the palettes? NO
			}
		}
		return `<g class="COLRv0 g${this.id}">\n${elements}</g>\n`;
	}
	else {
...
SamsaGlyph.prototype.svg = function (context={}) {
	...
	// we could cache arrays with glyphs that have COLRv1 and COLRv0 layers, but let’s check in realtime first
	// it’s not bad, since we check for the presence of the COLR table first anyway
	
	//let baseGlyphTarget = colr.baseGlyphRecords[this.id]; // NO, we need to find glyphIds in there
	//let baseGlyphPaint = colr.baseGlyphPaints[this.id]; // NO, we need to find glyphIds in there
	...
	svgString += font.COLR ? this.svgGlyphCOLRv1(context) || this.svgGlyphCOLRv0() || this.svgGlyphMonochrome() : this.svgGlyphMonochrome(); // this the default auto mode // TODO: offer method to select a specific format
	...
	//return finalizeSVG(svgString, defs, transform, style);
