...
class SamsaBuffer extends DataView {
	...
	decodePaint(context = {}) {
		...
		*/
		...
		if (context.paintIds[paint.offset]) // we must not have seen this paintId before in this glyph, otherwise we have infinite recursion
			return; // what should we return?
		else
			context.paintIds[paint.offset] = true;
...
SamsaGlyph.prototype.paint = function (context={}) {
	...
	if (!offset) {
		...
		if (!context.rendering) context.rendering = {};
		context.paintIds = []; // keep track of the paint IDs we’ve used (must be reset for each glyph)
	...
	if (result) {
		...
		for (let i=0; i<numLayers; i++) {
			...
			if (layerGlyph.numberOfContours > 0) {
				...
				elements += `<path d="${layerGlyph.svgPath()}" fill="${font.hexColorFromU32(layerColor)}"/>\n`; // TODO: precalculate the hex strings for the palettes? NO
			}
		}
